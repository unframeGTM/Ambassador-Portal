...
export async function getRegistrationsForContact(contactId) {
  ...
  const result = await conn.query(`
    SELECT
      ...
      Referred_Lead_Title__c,
      Account__r.Name,
      Ambassador__r.Name,
      Ambassador__r.Ambassador_Tier__c,
      Approval_Date__c,
...
// Admin view: every registration across all ambassadors, with the ambassador's
// name/email so the dashboard can group and filter by person.
export async function getAllRegistrations() {
  ...
  const result = await conn.query(`
    SELECT
      ...
      CreatedDate
    FROM Ambassador_Registration__c
    ORDER BY Ambassador__r.Name NULLS LAST, CreatedDate DESC
  `);
  return result.records;
}

// Admin view: every opportunity tied to an ambassador (via OpportunityContactRole),
// with the ambassador so the dashboard can group them alongside registrations.
export async function getAllAmbassadorOpportunities() {
  const conn = await getSFConnection();
  const result = await conn.query(`
    SELECT
      Id, ContactId, Contact.Name,
      Opportunity.Id, Opportunity.Name, Opportunity.StageName,
      Opportunity.Next_Steps_Date__c, Opportunity.CloseDate, Opportunity.Amount,
      Opportunity.Account.Name
    FROM OpportunityContactRole
    WHERE ContactId IN (
      SELECT Ambassador__c FROM Ambassador_Registration__c WHERE Ambassador__c != null
    )
  ...
  return result.records.map(r => {
    ...
    return {
      rowId: r.Id,
      ambassadorId: r.ContactId,
      ambassadorName: r['Contact']?.Name || 'Unknown',
      Id: opp.Id,
      Name: opp.Name,
      StageName: opp.StageName,
      Next_Steps_Date__c: opp.Next_Steps_Date__c,
...
export async function createRegistration({ contactId, accountId, accountName, accountWebsite, tier, notes, status, referredLeadTitle }) {
  const conn = await getSFConnection();
  ...
  const result = await conn.sobject('Ambassador_Registration__c').create({
    Ambassador__c: contactId,
    Account__c: accountId,
    Tier__c: tier,
    Notes__c: notes,
    Status__c: status || 'Pending Approval',
    Referred_Lead_Title__c: referredLeadTitle,
  });
...
export async function searchAccounts(query) {
  ...
  const result = await conn.query(
    `SELECT Id, Name, Website, Registration_Active__c, Registered_Ambassador__r.Name,
       (SELECT Intro_Meeting_Date__c FROM Ambassador_Registrations__r
        WHERE Status__c IN ('Approved', 'Intro Made', 'Active')
  ...
  return result.records.map(a => ({
    ...a,
    Active_Intro_Meeting_Date: a.Ambassador_Registrations__r?.records?.[0]?.Intro_Meeting_Date__c || null,
  }));
}
